...
use read_fonts::{FontRef, ResolveOffset, TableProvider, tables::gdef::CaretValue, types::GlyphId};
use serde::{Deserialize, Serialize};
use std::collections::BTreeSet;

pub const INTERNAL_CARET_AUTHORITY_REVISION: &str =
    "chaptera.internal-caret-authority.gdef-format1.v1";
pub const GDEF_FORMAT1_AUTHORITY_SOURCE: &str = "opentype_gdef_ligature_caret_format1";
...
pub fn resolve_internal_carets_ltr(
    ...
    for cluster in &clusters {
        let boundaries = eligible
            .iter()
            .copied()
            .filter(|boundary| cluster.start_scalar < *boundary && *boundary < cluster.end_scalar)
            .collect::<Vec<_>>();
...
mod tests {
    ...
    fn scalar_count_alone_never_creates_combining_or_zwj_internal_stops() {
        ...
        let zwj = resolve_internal_carets_ltr("f\u{200d}i", 0, &runtime(font), &[]).expect("shape");
    ...
    fn ffi_maps_two_policy_approved_boundaries_to_two_explicit_gdef_carets() {
        ...
        assert_eq!(
            [10, 11],
            [
                result.stops[0].scalar_boundary,
                result.stops[1].scalar_boundary
            ]
        );
        assert_eq!(
            [334, 668],
    ...
    fn duplicate_or_out_of_run_policy_boundaries_are_invalid_input() {
        let font = font_test_data::NOTOSERIF_AUTOHINT_SHAPING;
        assert_eq!(
            Err(InternalCaretAuthorityError::DuplicateEligibleBoundary { scalar_boundary: 1 }),
            resolve_internal_carets_ltr("fi", 0, &runtime(font), &[1, 1])
        );
        assert_eq!(
            Err(InternalCaretAuthorityError::EligibleBoundaryOutsideRun { scalar_boundary: 2 }),
