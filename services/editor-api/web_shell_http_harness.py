...
class HarnessState:
    def __init__(self) -> None:
        ...
        self.redo_operations: list[dict] = []
    ...
    def _scene_from_project(self, project: dict, revision_id: str) -> dict:
        ...
        for operation in project["operations"]:
            if operation.get("kind") != "move_node":
                raise ValueError("synthetic harness only supports MoveNode project replay")
            node = next(
                (
                    item
                    for item in scene["nodes"]
                    if item["node_id"] == operation["node_id"]
                ),
                None,
            )
            if node is None:
                raise ValueError("project operation targets unknown node")
    ...
    def executor(self, project: dict, command: dict):
        ...
        node = next(
            (
                item
                for item in base_scene["nodes"]
                if item["node_id"] == command["node_id"]
            ),
            None,
        )
        if node is None:
        ...
        next_project["operations"] = list(next_project["operations"]) + [
            copy.deepcopy(operation)
        ]
        self.redo_operations.clear()
        return operation, next_project, []

    def history_executor(self, project: dict, transition_kind: str):
        self.history_executor_calls += 1
        next_project = copy.deepcopy(project)
        operations = list(next_project["operations"])
        if transition_kind == "undo":
            if not operations:
                raise ValueError("nothing_to_undo")
            self.redo_operations.append(copy.deepcopy(operations[-1]))
            next_project["operations"] = operations[:-1]
            return next_project, [
        ...
        if transition_kind == "redo":
            ...
            next_project["operations"] = operations + [copy.deepcopy(operation)]
            return next_project, [
    ...
    def history(self, request: dict) -> dict:
        ...
        if result.get("protocol_version") == "chaptera.history-transition-accepted.v1":
            record = self.kernel.read_revision(
                document_id=self.document_id,
                revision_id=result["revision_id"],
            )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/health":
            self._json(
                {"ok": True, "receipt_class": "synthetic_http_service_plumbing"}
            )
            return
        if path == "/v1/scenes/current":
            with RECORDER.span("gateway.scene_current", trace_context):
                current = self._current_revision_id()
            ...
            self._json(
                {
                    ...
                    "redo_depth": len(STATE.redo_operations),
                }
            )
            return
    ...
    def _current_revision_id(self) -> str:
        return STATE.kernel.current_revision(STATE.document_id).revision_id

    def do_POST(self):
        path = unquote(self.path.split("?", 1)[0])
        trace_context = trace_context_from_headers(self.headers)
        if path not in {"/v1/commit", "/v1/history"}:
            self._json({"error": "not_found"}, 404)
            return
        try:
            ...
            if trace_context is not None and "client_operation_id" in request:
                ...
                trace_context["client_operation_id"] = request[
                    "client_operation_id"
                ]
            if path == "/v1/commit":
                ...
                with RECORDER.span("gateway.history", trace_context):
                    result = STATE.history(request)
            self._json(result)
