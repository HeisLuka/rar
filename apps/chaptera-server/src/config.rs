...
impl ChapteraConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        let admitted_max = u64::try_from(self.upload_admission.max_single_upload_bytes)
        ...
        if self.environment == EnvironmentMode::Prod {
            for (field, path) in [
                ("source_validation.isolation_python", &self.source_validation.isolation_python),
                ("source_validation.isolation_harness", &self.source_validation.isolation_harness),
                ("source_validation.worker_binary", &self.source_validation.worker_binary),
                ("source_validation.temp_root", &self.source_validation.temp_root),
            ] {
                if !path.is_absolute() {
                    return Err(ConfigError::new(
