...
impl SqliteJobQueue {
    ...
    async fn bootstrap(&self) -> Result<(), JobQueueError> {
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS jobs (
              ...
              job_kind               TEXT NOT NULL CHECK (job_kind IN ('parse','export','snapshot','projection','blob_gc')),
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS jobs_ready ON jobs(status, available_at_ms, created_at_ms, job_id)")
            .execute(&self.pool)
            .await
            .map_err(sqlite_error)?;
    ...
    pub async fn enqueue(&self, request: EnqueueRequest) -> Result<EnqueueOutcome, JobQueueError> {
        ...
        let result = sqlx::query(
            ...
            ) VALUES (?, ?, ?, ?, ?, ?, 'queued', ?, 0, ?, NULL, 0, NULL, NULL, ?, ?, NULL, NULL, NULL)
            "#,
        )
    ...
    pub async fn fail(
        ...
        if retryable && current.attempt < current.max_attempts {
            let backoff = retry_delay_ms(&current.job_id, current.attempt);
            let available = now_ms.saturating_add(backoff);
            let done = sqlx::query(
                r#"
                UPDATE jobs
                SET status='queued', available_at_ms=?, lease_owner=NULL, lease_expires_at_ms=NULL, terminal_code=?
    ...
    pub async fn publish_success(
        ...
        let result = async {
            ...
            if current.status == JobStatus::Succeeded {
                ...
                return Err(JobQueueError::new("effect_identity_conflict", "job succeeded with different effect identity"));
            }

            assert_live_lease(&current, lease, now_ms)?;
            if current.cancel_requested_at_ms.is_some() {
                return Err(JobQueueError::new("cancel_requested", "job is cancelled before publish barrier"));
            }
            ...
            .execute(&mut *conn)
            .await
            .map_err(|error| JobQueueError::new("effect_publish_failed", bounded_sqlx(&error)))?;
            ...
            .map_err(sqlite_error)?;
            if done.rows_affected() != 1 {
                return Err(JobQueueError::new("stale_lease", "success publication lost lease ownership"));
            }
...
fn decode_job(row: sqlx::sqlite::SqliteRow) -> Result<JobRecord, JobQueueError> {
    let bytes = |name: &str| -> Result<Vec<u8>, JobQueueError> {
