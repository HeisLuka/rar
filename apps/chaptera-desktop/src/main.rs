...
use chaptera_scene_instance::{
    GeometrySyncPolicyV1, ObjectMutationKindV1, admit_object_mutation_v1,
...
const APP_TITLE: &str = "Chaptera Editor — Desktop V0";
...
impl ViewerApp {
    ...
    fn sync_visual_geometry_from_editor(&mut self) {
        ...
        for scene_node in &mut visual.scene.nodes {
            ...
            // A projected visual instance must never inherit page-owned mutation
            // merely because its semantic origin belongs to some PAGE object.
            // Direct V0 geometry sync requires the source-authored parent to be
            // exactly the visual target page, then reuses the current Rar
            // SceneInstance admission law.
            if authored_node.header.parent_id != scene_node.parent_origin {
                continue;
            }

            let origin_node_id = scene_node.origin.as_canonical().to_string();
            let target_page_id = scene_node.parent_origin.to_string();
            let Ok(instance) =
                direct_page_local_instance_v1(&origin_node_id, &target_page_id)
            else {
                continue;
            };
    ...
    fn commit_canvas_drag(&mut self, drag: MoveTransaction) {
        ...
        let target_page = self
            .visual
            .as_ref()
            .and_then(|visual| visual.document.pages.get(self.selected_page))
        ...
        let outcome = self
            ...
            .and_then(|editor| {
                let target_page = target_page
                    .ok_or_else(|| "Move rejected: selected target page is unavailable.".to_owned())?;
                let authored_node = editor
                    .graph()
                    .nodes
                    .get(&drag.node_id())
                    .ok_or_else(|| "Move rejected: authored node is unavailable.".to_owned())?;

                if authored_node.header.parent_id != target_page {
                    return Err(
                        "Move rejected: visual instance is projected/read-only in Desktop V0."
                            .to_owned(),
                    );
                }

                let origin_node_id = drag.node_id().as_canonical().to_string();
                let target_page_id = target_page.to_string();
                let instance = direct_page_local_instance_v1(
                    &origin_node_id,
                    &target_page_id,
                )
                .map_err(|error| format!("Move rejected by SceneInstance gate: {error}"))?;
                let admission =
                    admit_object_mutation_v1(&instance, ObjectMutationKindV1::MoveNode);
                if !admission.admitted {
                    return Err(format!(
                        "Move rejected by SceneInstance gate: {}",
                        admission.reason
                    ));
                }
            ...
            .map(|editor| {
                page_nodes
                    ...
                    .filter_map(|scene_node| {
                        ...
                        // Direct page-local is a stronger condition than the
                        // historical "parent exists in graph.pages" predicate.
                        // Inherited master and Cmo carrier nodes retain their
                        // original source parent, so they fail closed here.
                        if authored_node.header.parent_id != page_origin {
                            return None;
                        }

                        let origin_node_id = scene_node.origin.as_canonical().to_string();
                        let target_page_id = page_origin.to_string();
                        let instance =
                            direct_page_local_instance_v1(&origin_node_id, &target_page_id).ok()?;
                        if !admit_object_mutation_v1(
                            &instance,
