...
const makeObs = () => new BrowserObservabilityV1({sessionIncarnation:"session:12345678",browserFamily:"chromium",idFactory:(p)=>p+":12345678"});
...
test("disabled mode emits no counters or spans", () => {
  const obs = makeObs(); const t = new RendererTelemetryV1({observability:obs,mode:"disabled"});
...
test("full mode extends existing trace spine", () => {
  const obs = makeObs(); const t = new RendererTelemetryV1({observability:obs,mode:"full"});
...
test("unsupported GPU and present timings stay explicit unknown", () => {
  const r = new RendererTelemetryV1({observability:makeObs(),mode:"counters"}).receipt(ctx.trace_id);
