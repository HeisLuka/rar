...
import argparse, json, pathlib, sys, tempfile, zipfile
...
from run_local_editor_desktop_vertical import (
    SAMPLE_SOURCE_BYTE_LEN, SAMPLE_SOURCE_HASH, bind_fixture, run_local_desktop_vertical,
)
...
def main() -> int:
    ...
    try:
        ...
        fixture = bind_fixture(args.fixture, expected_hash=SAMPLE_SOURCE_HASH, expected_len=SAMPLE_SOURCE_BYTE_LEN)
        with tempfile.TemporaryDirectory(prefix="chaptera-package-smoke-") as tmp:
            root = pathlib.Path(tmp)
            exe = extract_binary(pathlib.Path(request["zip_path"]), request["binary_entry"], root / "Chaptera.exe")
            receipt = run_local_desktop_vertical(
                fixture=fixture,
                project_output=root / "desktop-v0.project.json",
                export_output=root / "desktop-v0.edited.idml",
                receipt_output=root / "editor-desktop-vertical.real.json",
                command_template=[str(exe), "--desktop-acceptance-v1", "{fixture}", "{project}", "{export}"],
                rar_commit=args.rar_commit,
            )
        history = receipt["history"]
        proof = {
            "fixture_kind": "real_pub_sanitized",
            "source_sha256_before": SAMPLE_SOURCE_HASH,
            "source_sha256_after": SAMPLE_SOURCE_HASH,
            "reader_only": False,
            ...
            "supported_object_dragged": receipt["object_move"]["capability_admitted"] and receipt["object_move"]["durable_move_count"] == 1,
            "undo_redo_verified": history["undo_state_id"] == history["after_story_state_id"] and history["redo_state_id"] == history["after_move_state_id"],
            "editor_project_saved": receipt["project"]["operation_count"] == 2,
            "close_reopen_reproduced_state": history["reopened_state_id"] == history["after_move_state_id"],
            "unsupported_actions_fail_closed": receipt["invariants"]["projected_object_mutation_fails_closed"],
        }
